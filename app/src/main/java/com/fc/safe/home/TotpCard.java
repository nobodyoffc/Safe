...
public class TotpCard extends LinearLayout {
    ...
    public TotpCard(Context context, Secret detail, long currentTime) {
        super(context);
    ...
    private void init(Context context, Secret detail, long currentTime) {
        this.secret = detail;
    ...
    private String generateOpt(Secret detail, long currentTime) {
        try {
    ...
    private void showPopupMenu(Context context) {
        ...
        popup.setOnMenuItemClickListener(item -> {
            if (item.getItemId() == 1) {
                ...
                SecretManager.getInstance(context.getApplicationContext()).removeSecretDetail(secret);
                SecretManager.getInstance(context.getApplicationContext()).commit();
                ToastUtils.showInfo(context, context.getString(R.string.deleted));
                return true;
            } else if (item.getItemId() == 2) {
                // Show Detail: launch DetailActivity with secret
                Intent intent = new Intent(context, DetailActivity.class);
                intent.putExtra(DetailActivity.EXTRA_ENTITY_JSON, secret.toJson());
                intent.putExtra(DetailActivity.EXTRA_ENTITY_CLASS, secret.getClass().getName());
    ...
    public void refreshPassword(long currentTime) {
        optValue = generateOpt(secret, currentTime);
        if (isOptVisible) {
            optView.setText(optValue);
        }
    }
